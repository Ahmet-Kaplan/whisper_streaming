#!/usr/bin/env python3
"""Test audio receivers directly without importing the main package."""
...
def test_alsa_receiver():
    ...
    try:
        ...
        # Only test creation on Linux, otherwise just test import
...
def test_cross_platform_logic():
    ...
    try:
        ...
        if receiver_class.__name__ == expected or receiver_class.__name__ == "PyAudioReceiver":
            print(f"✓ Appropriate receiver selected for {system}")
        else:
            print(f"⚠ Unexpected receiver {receiver_class.__name__} for {system}")
